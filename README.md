...
Speed splits four ways ([docs/SCALE.md](docs/SCALE.md)): Pixeltable is last on ingest
(10.45x realtime against Supabase's 16.89x at 100 videos) and last on search by ~7ms,
but 3.4x faster than Supabase on the agent query, which costs the other two three round
trips to their compute service and Pixeltable none. Convex is fastest on reads; under
8 concurrent clients Pixeltable's in-process model serializes and the order inverts.
...
(`cd compute-service && pip install -e . && uvicorn app:app --port 9000`), then
