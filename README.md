...
and on frame search, and answers the agent query in a fraction of the others' time, a
gap that is mostly the device each library defaults to for the chat model: Metal in
Pixeltable's UDF, CPU in `llama-cpp-python` ([docs/device.json](docs/device.json)). Convex is
fastest on reads, and under eight concurrent clients Pixeltable degrades most.
