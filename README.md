...
- **Media processing has to live somewhere else.** Neither Deno nor the Convex runtime
  runs ffmpeg, so both need `compute-service/`; three of its seven endpoints have no
  hosted-API substitute.
- **Adding a column to live data** is 1 line against 24 and 53
...
- **Errors are per cell** (`errormsg`, `errortype`); elsewhere a failed step is a NULL.
