...
http.route({
  ...
  handler: httpAction(async (ctx, req) => {
    const body = await req.json();
    return json(await ctx.runAction(api.search.searchFrames, { query: body.query, limit: body.limit }));
  }),
});
...
http.route({
  ...
  handler: httpAction(async (ctx, req) => {
    const body = await req.json();
    return json(
      await ctx.runAction(api.search.searchTranscripts, { query: body.query, limit: body.limit }),
    );
  }),
});
