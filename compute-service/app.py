...
class EmbedClipResponse(BaseModel):
...
async def extract_frames(req: ExtractFramesRequest):
    ...
    return ExtractFramesResponse(frames=frames_b64, count=len(frames_b64))
...
@app.post('/embed-clip', response_model=EmbedClipResponse)
async def embed_clip(req: EmbedClipRequest):
    ...
    return EmbedClipResponse(embeddings=embeddings, dimensions=dims)
...
async def embed_text(req: EmbedTextRequest):
    ...
    return EmbedTextResponse(embeddings=vectors, dimensions=len(vectors[0]) if vectors else 0)
...
async def detect_scenes(req: DetectScenesRequest):
    ...
    return DetectScenesResponse(scenes=scenes, count=len(scenes))
