...
Creates 3 videos (~5-8 seconds each) with:
...
def _generate_tts(text: str, outpath: Path) -> None:
    ...
    try:
        ...
        print('gTTS not installed — generating silent audio instead')
        subprocess.run(
            ['ffmpeg', '-y', '-f', 'lavfi', '-i', 'anullsrc=r=22050:cl=mono', '-t', '5', str(outpath)],
            capture_output=True,
            check=True,
        )
        return
...
def _generate_video(
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        # Build segment filter for colour backgrounds with text
        ...
        for i, seg in enumerate(segments):
            ...
            inputs.append(
                f"color=c={color}:s=640x360:d={seg_dur:.2f},"
        ...
        cmd = [
            'ffmpeg', '-y',
            '-f', 'lavfi', '-i', f'color=c=black:s=640x360:d={audio_dur:.2f}',
            '-i', str(audio_path),
